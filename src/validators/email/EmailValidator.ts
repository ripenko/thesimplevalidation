...
export class EmailValidator<TModel, K extends keyof TModel> extends Validator<
  TModel,
  K,
  EmailValidatorSetup<TModel, K>
> {
  private localAtom = /^[A-Za-z0-9!#$%&'*+/=?^_`{|}~-]+$/;
  private domainLabel = /^[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?$/;
  ...
  public async isValidInternal(
    ...
    const isValid: boolean = this.isEmailAsciiStrict(value);
  ...
  private isEmailAsciiStrict(value: unknown): boolean {
    ...
    if (at <= 0) return false; // no local-part or '@' is the first character
    if (value.indexOf("@", at + 1) !== -1) return false; // multiple '@'
    ...
    if (localParts.some((p) => p.length === 0)) return false;

    // Each atom must contain only allowed ASCII characters
    if (
      localParts.some((p) => !(this.setup.localAtom ?? this.localAtom).test(p))
    )
      return false;
    ...
    if (
      labels.some((l) => !(this.setup.domainLabel ?? this.domainLabel).test(l))
    )
      return false;
